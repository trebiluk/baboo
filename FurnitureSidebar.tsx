...
export function FurnitureSidebar() {
  ...
  if (tool !== 'furniture' && tool !== 'select') {
    return (
      <aside className="side-panel furniture-side collapsed-hint">
        <p>Switch to <strong>Furniture</strong> to browse the catalog.</p>
      </aside>
    );
  }
  ...
  return (
    <aside className="side-panel furniture-side">
      <h2>Furniture</h2>
      <p className="muted">Click an item, then click the plan — or drag onto the canvas. Snap is optional (Settings).</p>
      {cats.map((cat) => (
